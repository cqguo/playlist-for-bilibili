...
| 批量选择多分 P 或合集条目加入列表 | 自动去重并保留每个分 P 的标题 |
| 编辑视频标题，移除不需要的项目 | 从列表中任意视频开始播放 |
| 单个列表最多保存 2000 个视频 | 每个列表独立记忆播放模式 |
...
| 批量添加分 P / 合集 | 多选当前多分 P 或合集中的条目并一次加入列表 |
...
当当前视频属于多分 P 视频或合集时，扩展菜单会显示“批量添加分 P / 合集”：

1. 点击“批量添加分 P / 合集”。
2. 勾选多个条目，也可以使用“全选”。
3. 点击“选择播放列表”，再选择目标列表。

已存在的条目不会重复添加；标题有变化时会自动更新。操作完成后会显示新增、更新、跳过和因容量上限未添加的数量。
